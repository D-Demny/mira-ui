...
describe('resolveOfflineScreen', () => {
  ...
  it('shows the USB-tethering onboarding the user picked (ticket10-6C)', () => {
    // first boot, nothing to go on -> the pick wins over the chooser guess
    expect(resolveOfflineScreen(input({ method: 'tethering-onboarding' }))).toBe(
      'tethering-onboarding',
    )
    expect(
      resolveOfflineScreen(input({ method: 'tethering-onboarding', setupOverride: true })),
    ).toBe('tethering-onboarding')
  })
