...
describe('resolveRoute', () => {
  ...
  describe('offline', () => {
    ...
    // ticket10-6C fork branch: the USB-tethering onboarding wizard
    it('passes the tethering-onboarding screen through', () => {
      expect(resolveRoute(input({ offlineScreen: 'tethering-onboarding' }))).toEqual({
        kind: 'offline',
        screen: 'tethering-onboarding',
      })
    })
  })

  describe('library', () => {
    // fork branch: library navigation replaces idle/playing while open
    it('replaces the player when a device is active', () => {
      expect(resolveRoute(input({ showingLibrary: true }))).toEqual({ kind: 'library' })
    })

    it('replaces idle when nothing is playing', () => {
      expect(resolveRoute(input({ status: idle, showingLibrary: true }))).toEqual({
        kind: 'library',
      })
    })

    it('is not suppressed by reconnecting — user navigation, not daemon state', () => {
      // without the library flag a drop on an active device holds the player;
      // with it open the library stays up
      expect(resolveRoute(input({ status: idle, reconnecting: true })).kind).toBe('player')
      expect(
        resolveRoute(input({ status: idle, reconnecting: true, showingLibrary: true })),
      ).toEqual({ kind: 'library' })
    })

    it('still loses to the rungs above it', () => {
      const over = { showingLibrary: true }
